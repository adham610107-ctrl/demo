...
function createParticles(event) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        const dx = (Math.random()-0.5)*110, dy = (Math.random()-0.5)*110;
    ...
    // Yashil porox effekti
    createGreenSparkle(x, y);
}

function createGreenSparkle(x, y) {
    const COUNT = 22;
    for (let i = 0; i < COUNT; i++) {
        ...
        setTimeout(() => {
            ...
            spark.style.cssText = `left:${x}px;top:${y}px;width:${size}px;height:${size}px;background:radial-gradient(circle,#AFFFB0,#00C853);box-shadow:0 0 6px 2px rgba(0,200,83,0.7);transform:translate(-50%,-50%);`;
            spark.animate(
                [{transform:`translate(-50%,-50%) scale(1)`, opacity:1},
                 {transform:`translate(calc(-50% + ${dx}px),calc(-50% + ${dy}px)) scale(0.2)`, opacity:0}],
                {duration: 600 + Math.random()*400, easing:'cubic-bezier(0,0.9,0.57,1)', fill:'forwards'}
            );
...
function goHome() {
    ...
    // Streak qaytadan ko'rsatiladi
    const streakBox = document.getElementById('streak-nav-box');
    if (streakBox) streakBox.style.display = '';
...
function startTestSession(){
    ...
    // Streak navbarda test paytida yashiriladi
    const streakBox = document.getElementById('streak-nav-box');
...
async function sendAdminMessage() {
    ...
    if (btn) { btn.disabled=true; btn.innerText='Yuborilmoqda...'; }
    const isBroadcast = ['all','hammaga','hamma'].includes(to.toLowerCase());
    try {
        if (isBroadcast) {
